...
module SpeculativeDecodingExample
  ...
  class RelativePositionBias < MLX::NN::Module
    ...
    def call(query_length, key_length, offset: 0)
      ...
      bucket = SpeculativeDecodingExample.relative_position_bucket(
        relative_position,
        bidirectional: @bidirectional,
  ...
  class TransformerDecoder < MLX::NN::Module
    ...
    def call(x, memory, cache: nil)
      ...
      t = x.shape[1]
      mask = if t > 1
        SpeculativeDecodingExample.create_additive_causal_mask(t, offset: offset)
      else
        nil
      end
  ...
  class Model < MLX::NN::Module
    ...
    def decode(inputs, memory)
      inputs = wte.call(inputs)
      y, self.cache = decoder.call(inputs, memory, cache: cache)
      if !tie_word_embeddings
        y = MLX::Core.multiply(y, @model_dim**-0.5)
        y = lm_head.call(y)
      else
        y = MLX::Core.matmul(y, wte.weight.T)
      end
      y
    end

    def call(inputs, decoder_inputs)
      decoded = decode(decoder_inputs, encode(inputs))
      first = MLX::Core.take(decoded, MLX::Core.array([0], MLX::Core.int32), 0)
      MLX::Core.squeeze(first, 0)
    end
  end
end
