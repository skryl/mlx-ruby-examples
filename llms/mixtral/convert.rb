...
if $PROGRAM_NAME == __FILE__
  default_params = Pathname.new(__dir__).join("params.json").to_s
  options = {
    torch_path: "Mixtral-8x7B-v0.1",
    mlx_path: "mlx_model",
  ...
  parser = OptionParser.new do |opts|
    opts.banner = "Usage: ruby llms/mixtral/convert.rb [options]"
    opts.on("--torch-path PATH", String, "Path to PyTorch model directory") { |v| options[:torch_path] = v }
    opts.on("--mlx-path PATH", String, "Output MLX directory") { |v| options[:mlx_path] = v }
    opts.on("--params-path PATH", String, "Mixtral params json path") { |v| options[:params_path] = v }
  ...
  args = [
    ...
    "--params-path", options[:params_path],
  ...
  unless status.success?
    raise RuntimeError, "mixtral convert failed:\n#{stderr}"
  end
